read_surveyxact_csv <- function(file, header, col_names, trim_ws = TRUE) {
  suppressWarnings(
    utils::read.delim(
      file = file,
      header = header,
...
read_surveyxact <-
  function(
    ...
    for (col in names(df_data)) {
      if (is.integer(df_data[[col]]) || is.logical(df_data[[col]])) {
        df_data[[col]] <- as.numeric(df_data[[col]])
      }
      if (
        is.numeric(df_data[[col]]) &&
          suppressWarnings(max(nchar(df_data[[col]]), na.rm = TRUE) > 8)
      ) {
    ...
    if (length(unmatched_structure) > 0L) {
      cli::cli_warn(c(
        i = "Unable to find following Structure-variables in Dataset-variables:",
        i = "{rlang::expr_text(unmatched_structure)}."
      ))
    }
