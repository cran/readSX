...
check_filepath <- function(filepath = filepath) {
  ...
  if (!inherits(x = filepath, what = "character")) {
    cli::cli_abort(c(
      "{.arg filepath} must be of type `character` or `fs_path` vector.",
  ...
  if (!all(files_exist)) {
    cli::cli_abort(c(
      "{length(files_exist[!files_exist])} file{?s} not found:",
  ...
  if (
    length(filepath) == 1L && grepl("\\.xlsx$", filepath, ignore.case = TRUE)
  ) {
  ...
  if (
    length(filepath) == 3L &&
      all(grepl("\\.csv$", filepath, ignore.case = TRUE))
  ) {
    if (
      !is.null(names(filepath)) &&
      ...
      if (any(nchar(filepath) == 0L)) {
        cli::cli_abort(c(
          "Unable to guess roles from filenames.",
